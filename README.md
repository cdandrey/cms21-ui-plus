...
|---|---|---:|---|
| **Part repairability badge** | `showPartRepairabilityIndicators` | `true` | Adds a white wrench to repairable loose-part rows in inventory, warehouse, barn, junkyard and part-selection windows including repair, scrap, upgrade and brake lathe. When CMS21 Gameplay+ is installed, all required specialization icons are stacked below the wrench in the same order as the Skills window, while the required Restorer level remains beside the wrench. The integration stays optional and does not add a DLL dependency between the mods. |
| **Owned-part count badge** | `showOwnedPartCountIndicators` | `true` | Shows the warehouse-count badge on grouped inventory and warehouse packages; single-copy rows in the normal inventory and both warehouse tabs omit the numeric badge. Other supported part-selection cards keep their existing owned-part availability indication. Wheel identity includes ET/profile/size/width, and components inside a stored assembly are not counted again as loose copies. |
| **Mount/unmount part card badges** | `showMountPartCardIndicators` | `true` | Adds repairability and owned-part availability badges directly to the hovered part card while mounting or removing parts. Repairability is shown in the upper-left corner; with CMS21 Gameplay+ installed, all required specialization icons appear below the wrench and the required Restorer level appears to its right. Availability is shown in the upper-right corner with the same condition-coloured owned-count breakdown used by inventory and warehouse rows, or a red warehouse badge when no matching part is owned. |
...
| **Group identical parts** | `groupInventoryParts` | `true` | Collapses matching inventory and warehouse entries, including barn maps and cases, into expandable packages. Loose parts group by ID, rims and tires also by dimensions, and complete assemblies by their own ID plus component identities. Condition, quality, paint colour and licence-plate text do not split a package. Active quick filters decide whether a package is visible and which members it contains; the package/single-position quick filter is hidden while a package is open. Double left-click opens a package; right-click closes it. Barn-map and case packages are excluded from whole-package sale. Selling another collapsed package sells every visible member for the combined native sale value, and moving it between inventory and warehouse moves every visible member together. Loose-part packages are shown as a three-thumbnail stack without additional 3D rendering. |

### Inventory filters

| In-game setting | Config flag | Default | Detailed behavior |
|---|---|---:|---|
| **Inventory quick filters** | `addInventoryQuickFilters` | `true` | Adds condition, repairability, quality and package/single-position filters to garage inventory and warehouse lists, plus owned/missing filtering in barn and junkyard. |
...
| **Repair inventory filters** | `addRepairInventoryFilters` | `true` | Adds an immediate localized-name search plus condition, restoration-availability and quality filters to both the mechanical-part repair inventory and the body-panel repair inventory. Restoration availability can show all parts, only parts allowed by the player's current repair skills, or only parts currently blocked by those skills. When CMS21 Gameplay+ is installed, it supplies the current restoration-availability state used by this filter and owns the repair-lock presentation. The filtered result is rebuilt after a repair animation completes, and its state is independent from the scrap filters. |
| **Reset repair filters on exit** | `resetRepairInventoryFiltersOnExit` | `true` | Clears repair condition, restoration-availability, quality and search state when the repair window closes. Disabling it keeps the state for the next opening in the same scene. |
...
#### Filter states and condition ranges

- Condition wherever this quick filter is available: off → **white** (15–100%) → red (0–14%) → orange (15–49%) → yellow (50–79%) → green ring (80–99%) → green (100%). Upgrade has no condition quick filter.
...
- Garage inventory and warehouse package state: off → packages only → single positions only. The package state is calculated after the other active quick filters, so a name with one remaining matching copy is treated as a single position.

Left click opens a drop-down list of the states available for that quick filter. A second left click on a state selects it. Any other mouse click or standard UI cancel, submit or navigation command closes the list without changing the filter. Supported quick-filter windows show a left-mouse-button `SET FILTER` footer hint before the `LeftAlt` reset hint.
...
Whenever a numeric owned-count badge is shown, a white total is displayed above the non-empty coloured condition counts, including when every owned copy belongs to the same condition range. The repairability and owned-count badges are independent of each other and remain available when quick filters are disabled.
...
With **Improved shopping list** enabled, twelve compact shop-category icons filter the list by Main, Body, Rims, Tires, Interior, Licence Plates, Gearbox, Tuning, Body Tuning, Electronics, Community and Addons; enabled categories are white and disabled categories are grey. The icons reuse the native shop-selection sprites. A single left-click toggles one category. Double left-click leaves only that category enabled when it was disabled or shared the active set; when it was the only enabled category, double left-click disables it and enables all others. With `filterShoppingListByCurrentShopOnOpen` enabled, opening the list inside a specific parts shop enables only that shop category; otherwise the list opens with all categories enabled, as it does from inventory or the shop hub. Pressing `Alt` toggles all categories on or off. The shopping list is presented as a compact inventory-style card grid sized to 75% of the native inventory card. The number of columns is calculated from the available list width with a 2-pixel gap between cards and equal leftover margins on the left and right; when the whole list fits on one row, cards start from the left with the same 2-pixel edge gap. Each card shows the part thumbnail and localized name without a condition bar; tire and rim parameters are displayed directly on the card. Quantity, `-` / `+` controls and the native delete action are placed directly below the card; `-` is disabled when only one item remains. The same quantity changes are available from the `-` / `+` keyboard keys while a shopping-list card is selected; holding `X` keeps the native clear-all shopping-list action. Inside a specific parts shop, holding `Space` opens a native confirmation for buying every compatible shopping-list entry sold by that shop and reports the matching part count and discounted cost before purchase. The action is hidden on the shop hub; tire and rim dimensions are preserved, tuning shops buy available tuning variants, and successfully purchased quantities are removed from the list. Repairability and warehouse indicators are drawn in the upper-left and upper-right corners of each card; with CMS21 Gameplay+ installed, all required specialization icons are shown below the wrench and the required Restorer level is shown to its right. Owned quantities are always shown vertically below the warehouse icon as total, 100%, 50–99%, 15–49% and below-15% counts, omitting empty condition groups; missing parts keep only the red warehouse icon.
