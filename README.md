...
| **Improved shopping list** | `addShoppingListSorting` | `true` | Enables shop-category quick filters, the two-column presentation, `-` / `+` quantity controls, repairability and warehouse-availability badges, persistent sorting menu and hold-`Space` bulk purchase for the current shop. Sorting supports name, warehouse availability, unit price and repairability in both directions. |
...
With **Improved shopping list** enabled, twelve compact shop-category icons filter the list by Main, Body, Rims, Tires, Interior, Licence Plates, Gearbox, Tuning, Body Tuning, Electronics, Community and Addons; enabled categories are white and disabled categories are grey. The icons reuse the native shop-selection sprites. With `filterShoppingListByCurrentShopOnOpen` enabled, opening the list inside a specific parts shop enables only that shop category; otherwise the list opens with all categories enabled, as it does from inventory or the shop hub. Pressing `Alt` toggles all categories on or off. The native shopping list is presented in two visual columns while its native three-column selection mapping is preserved for navigation. The native `X` action and row delete icon still remove the entire selected entry. Adjacent `-` / `+` controls decrease or increase the requested quantity by one; `-` is disabled when only one item remains. The same quantity changes are available from the `-` / `+` keyboard keys while a shopping-list row is selected. Inside a specific parts shop, holding `Space` opens a native confirmation for buying every compatible shopping-list entry sold by that shop and reports the matching part count and discounted cost before purchase. The action is hidden on the shop hub; tire and rim dimensions are preserved, tuning shops buy available tuning variants, and successfully purchased quantities are removed from the list. Repairability and warehouse indicators are placed immediately before the purchase amount; owned-part condition counts expand to the left on hover, temporarily reduce and clip the part-name area to prevent overlap, and collapse back to the warehouse icon when the pointer leaves the indicator/details area. Missing parts keep only the red warehouse icon and do not display zero counts.

Shopping-list ownership sorting uses the same shared part-identity rules as the ownership badges, including wheel dimensions. The purchase helper stores the selected entry only briefly and applies it only when the opened shop item is compatible, preventing an old selection from being injected into an unrelated purchase.
