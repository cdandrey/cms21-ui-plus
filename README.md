...
|---|---|---:|---|
| **Part repairability badge** | `showPartRepairabilityIndicators` | `true` | Adds a condition-coloured wrench to repairable loose-part rows in the inventory, warehouse, barn, junkyard, scrap, scrap-upgrade and mechanical/body repair lists. Shopping-list entries use a green wrench for repairable parts and a red wrench for non-repairable parts. The badge follows the effective repairability of the part. When CMS21 Gameplay+ is also installed, its optional brake-drum repair support is reflected without making either mod a required dependency of the other. |
| **Owned-part count badge** | `showOwnedPartCountIndicators` | `true` | Shows how many matching loose parts or complete assemblies are owned across the normal inventory and every unlocked warehouse. Counts are grouped by condition, wheel identity includes ET/profile/size/width, and components inside a stored assembly are not counted again as loose copies. In the shopping list, a white warehouse icon marks an owned match and a red warehouse icon marks a missing part; hovering an owned icon expands the white total and only the non-zero condition counts. |
...
The native shopping list is presented in two visual columns while its native three-column selection mapping is preserved for navigation. The native purchase amount and delete controls remain in place. Repairability and warehouse indicators are placed immediately before the purchase amount; owned-part condition counts expand to the left on hover, temporarily reduce and clip the part-name area to prevent overlap, and collapse back to the warehouse icon when the pointer leaves the indicator/details area. Missing parts keep only the red warehouse icon and do not display zero counts.
